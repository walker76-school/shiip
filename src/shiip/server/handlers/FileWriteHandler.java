...
public class FileWriteHandler extends WriteHandler {
    ...
    public FileWriteHandler(ClientConnectionContext connectionContext, FileContext fileContext, Logger logger) {
        super(connectionContext, logger);
        logger.log(Level.INFO, "new FileWriteHandler - " + fileContext.getStreamID());
    ...
    protected synchronized void handleWriteCompleted() {
        System.out.println("FileWriteHandler " + fileContext.getStreamID() + " completed - " + context.getBufferedMessages().size() + " remaining buffered messages");
